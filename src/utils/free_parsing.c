...
/*   Updated: 2023/07/18 17:55:22 by lgirault         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	free_parsing(t_data *data, char *str)
{
	...
	free(data->floor_color);
	free(data->sky_color);
	while (++i <= 3 && data->tab_color_floor != NULL
		&& data->tab_color_floor[i] != NULL)
		free(data->tab_color_floor[i]);
	free(data->tab_color_floor);
	i = -1;
	while (++i <= 3 && data->tab_color_sky != NULL
		&& data->tab_color_sky[i] != NULL)
		free(data->tab_color_sky[i]);
	free(data->tab_color_sky);
	printf("Error\n%s\n", str);
	exit(1);
